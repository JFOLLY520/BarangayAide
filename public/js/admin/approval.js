 console.log("approval.js for ADMIN");
...
function sort(){
  ...
  var docRef = db.collection("users").doc(uidGet); //takes uid of current user
    docRef.get().then(function(doc) {
      ...
      if (newest.checked) {
        db.collection("posts").where("accepted","==",false).where("status","==","Ongoing").where("barangayID","==",brgyID).orderBy("datetime","desc").limit(5)
          ...
          // doc.data() will be undefined in this case
          db.collection("posts").where("accepted","==",false).where("status","==","Ongoing").where("barangayID","==",brgyID).orderBy("datetime","asc").limit(5)
          .get()
          .then(function(querySnapshot) {
              querySnapshot.forEach(function(doc) {
                  // doc.data() is never undefined for query doc snapshots
...
function deletePost(postID){
  bootbox.confirm({
   ...
   function approvePost(postID){
      ...
      firebase.auth().onAuthStateChanged(firebaseUser=> {
        ...
        .then(function(querySnapshot) {
            querySnapshot.forEach(function(doc) {
                ...
                console.log("end: "+end);
                db.collection("posts").where("accepted","==",false).where("barangayName", "==", brgy).where("status", "==", "Ongoing").where("datetime",">",start).where("datetime","<",end).orderBy("datetime","desc")
                    .get()
                    .then(function(querySnapshot) {
    ...
    saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), 'Pending.xlsx');
    })
    });
    })
      }
        ...
        .then(function(querySnapshot) {
            querySnapshot.forEach(function(doc) {
                ...
                var barangayID = doc.data().barangayID;
                  ...
                  ref = db.collection("posts").where("accepted","==",false).where("status","==","Ongoing").where("barangayID","==",barangayID).orderBy("datetime","desc").limit(5)
                    .get()
                    .then(function(querySnapshot) {
                        querySnapshot.forEach(function(doc) {
                    ...
                    "<tr class='text-center'>"+
                     ...
                     "</tr>"
                            ...
                            //latestDoc = ref.docs[ref.docs.length - 1];
                        });
    ...
    firebase.auth().onAuthStateChanged(firebaseUser=> {
      ...
      .then(function(querySnapshot) {
          querySnapshot.forEach(function(doc) {
              ...
              //console.log(doc.id, " => ", doc.data().name, " => ", doc.data().barangayID);
              var barangayID = doc.data().barangayID;
              var ref = db.collection("posts").where("accepted","==",false).where("status","==","Ongoing").where("barangayID","==",barangayID).orderBy("datetime","desc").limit(5)
              ...
              console.log(doc.data().barangayID);
               db.collection("posts").where("accepted","==",false).where("status","==","Ongoing").where("barangayID","==",barangayID).orderBy("datetime","desc").startAfter(lastVisible).limit(5)
                  .get()
                  .then(function(querySnapshot) {
                      querySnapshot.forEach(function(doc) {
