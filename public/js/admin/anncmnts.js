...
//action buttons
/*
"<button type='button' class='btn btn-block' onclick='deleteAnncmnt(\""+doc.id+"\")'><span class='lnr lnr-trash'></span> Delete</button>"+
"<button type='button' id='resetBtn' class='btn btn-block' onclick='editAnncmnt(\""+doc.id+"\")' data-toggle='modal' data-target='#editAnncmntModal'>Edit</button>"+
*/
  ...
  firebase.auth().onAuthStateChanged(firebaseUser=> {
    ...
    .then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
            ...
            console.log("end: "+end);
            db.collection("posts").where("accepted","==",true).where("barangayName", "==", brgy).where("issue", "==", "Announcement").where("datetime",">",start).where("datetime","<",end).orderBy("datetime","desc")
                .get()
                .then(function(querySnapshot) {
        ...
        var ws_data = [['First Name','Last Name','Date','Description','Image URL']];
         ...
         const docSnapshots = querySnapshot.docs;
          ...
          ws_data.push([doc.fname, doc.lname, noTZ, doc.description,doc.images]);
...
firebase.auth().onAuthStateChanged((user) => {
  ...
  docRef.get().then(function(doc) {
    ...
    if (doc.exists) {
      db.collection("posts").where("accepted","==",true).where("barangayName", "==", brgy).where("issue", "==", "Announcement").orderBy("datetime","desc").limit(5)
