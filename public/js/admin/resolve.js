...
bootbox.confirm({
  ...
  var docRef = db.collection("users").doc(uidGet); //takes uid of current user
    docRef.get().then(function(doc) {
      ...
      if (newest.checked) {
        db.collection("posts").where("accepted","==",true).where("status","==","Ongoing").where("barangayID","==",brgyID).orderBy("datetime","desc").limit(5)
          ...
          db.collection("users").doc(doc.data().userID).get().then(function(querySnapshot1){
              ...
              dispResolved.innerHTML +=
                        "<tr>"+
                            ...
                            "<td><button type='button' class='btn btn-info btn-sm mr-1 approve-post' onclick='resolvePost(\""+doc.id+"\")'>✔ Resolve</button>"+
                            "<button type='button' class='btn btn-danger btn-sm' onclick='archivePost(\""+doc.id+"\")'>✖ Delete</button></td>"+
          ...
          // doc.data() will be undefined in this case
          db.collection("posts").where("accepted","==",true).where("status","==","Ongoing").where("barangayID","==",brgyID).orderBy("datetime","asc").limit(5)
          .get()
          .then(function(querySnapshot) {
              querySnapshot.forEach(function(doc) {
                  // doc.data() is never undefined for query doc snapshots
  ...
  firebase.auth().onAuthStateChanged(firebaseUser=> {
    ...
    .then(function(querySnapshot) {
        querySnapshot.forEach(function(doc) {
            ...
            console.log("end: "+end);
            db.collection("posts").where("accepted","==",true).where("barangayName", "==", brgy).where("status", "==", "Ongoing").where("datetime",">",start).where("datetime","<",end).orderBy("datetime","desc")
                .get()
                .then(function(querySnapshot) {
        ...
        wb.Props = {
                Title: "Ongoing Complaints Reports",
                Subject: "Reports",
                Author: "barangayName",
                CreatedDate: new Date(2017,12,19)
        };
        wb.SheetNames.push("Test Sheet");
        var ws_data = [['First Name','Last Name','Date','Issue','Barangay','Description','Image URL']];
         ...
         const docSnapshots = querySnapshot.docs;
          ...
          ws_data.push([doc.fname, doc.lname, noTZ, doc.issue, doc.barangayName, doc.description,doc.images]);
...
saveAs(new Blob([s2ab(wbout)],{type:"application/octet-stream"}), 'Ongoing.xlsx');
})
});
})
  }
...
firebase.auth().onAuthStateChanged(firebaseUser=> {
  ...
  .then(function(querySnapshot) {
      querySnapshot.forEach(function(doc) {
          ...
          db.collection("posts").where("accepted","==",true).where("status","==","Ongoing").where("barangayID","==",barangayID).orderBy("datetime","desc").limit(5)
              .get()
              .then(function(querySnapshot) {
                  querySnapshot.forEach(function(doc) {
    ...
    firebase.auth().onAuthStateChanged(firebaseUser=> {
      ...
      .then(function(querySnapshot) {
          querySnapshot.forEach(function(doc) {
              ...
              //console.log(doc.id, " => ", doc.data().name, " => ", doc.data().barangayID);
              var barangayID = doc.data().barangayID;
              var ref = db.collection("posts").where("accepted","==",true).where("status","==","Ongoing").where("barangayID","==",barangayID).orderBy("datetime","desc").limit(5)
              ...
              console.log(doc.data().barangayID);
               db.collection("posts").where("accepted","==",true).where("status","==","Ongoing").where("barangayID","==",barangayID).orderBy("datetime","desc").startAfter(lastVisible).limit(5)
                  .get()
                  .then(function(querySnapshot) {
                      querySnapshot.forEach(function(doc) {
