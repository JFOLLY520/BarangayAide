      ...
      if (doc.exists) {
        if(doc.data().role == 3){
          //console.log("Document data:", doc.data());
            console.log('Logged in as: ', user.email, '||', doc.data().fname, doc.data().lname);      
  ...
  btnUpload.addEventListener('change', function(e){
    ...
    task.on('state_changed',
      ...
      function error(err){
        bootbox.alert({
          title: "BarangayAide",
            message: "Your image is too large. Please upload an image at least 1 MB in size.",
...
function censorComment(postID){
  var hide = document.getElementById("hideCmmt"+postID);
          ...
          db.collection("feedback").where("postID", "==", post).where("commented","==", true).orderBy("datetime","asc")
              .onSnapshot(function(querySnapshot) {
                  querySnapshot.forEach(function(doc) {
                    ...
                    var cmmtStatus = db.collection("feedback").doc(commentID);
                      return cmmtStatus.update({
                          hidden: true
                      })
                      .then(() => {
  ...
  /*var hide_comment = db.collection("feedback").doc(postID);

  hide_comment.get().then(function(doc) {
      if (doc.exists) {
        hide_comment.update({
          hidden: true
        })
                     ...
                     db.collection("users").doc(doc.data().userID).get().then(function(querySnapshot1){
                      ...
                      "<div class='row mb-1'>"+
                        "<div class='col-md-12'>"+
                          ...
                          "<button type='button' class='mb-1' style=' font-size: 10px;' id='hideCmmt' onclick='censorComment(\""+postID+"\")'> <i class='bi bi-dash-circle-fill' style=' font-size: 12px;'></i> hide comment</button>"+
                          "</div>"+
                      ...
                      "<hr>"
                            ...
                            cmmtData.innerHTML += "This comment has been hidden because it violated the rules of the barangay.<hr> "

                     /* "<div class='row mb-1'>"+
                        "<div class='col-md-12'>"+
                          "<div class='container row'>"+
                          "<button type='button' style=' font-size: 10px;' id='hideCmmt' onclick='censorComment(\""+postID+"\")'> <i class='bi bi-dash-circle-fill' style=' font-size: 12px;'></i> hide comment</button>"+
                          "</div>"+
                          "<p> <h6>"+fname+" "+lname+"</h6> "+noTZ+"</p>"+
  ...
  if (user) {
    ...
    docRef.get().then((doc) => {
        if (doc.exists) {
          ...
          db.collection("posts").where("barangayID","==",brgyID)
            .get()
            .then((querySnapshot) => {
                querySnapshot.forEach((doc1) => {
                 
                  var postDate = doc1.data().datetime.toDate();
                  var year = postDate.getFullYear();
                  var month = postDate.getMonth();
                  var day = postDate.getDate();
                  var yearly = doc.data().explicitYears;
                            
                   var c = new Date(year + yearly, month, day);
                       // console.log(c);
