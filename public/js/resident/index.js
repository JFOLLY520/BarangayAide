...
var btnUpload = document.getElementById('btnUpload');
	...
	btnUpload.addEventListener('change', function(e){
        ...
        bootbox.alert({
          title: "BarangayAide",
            message: "Your image is too large. Please upload an image of least 1 MB in size.",
...
/*============================Barangay Filter================================*/
	...
	console.log(sortBrgy+""+sortIssue+""+sortStatus);
...
function tryfunction(){
	...
	db.collection("posts").where("accepted", "==", true)
    .get()
...
function newPost(){
	...
	if(dscrtpn =="" && slctdBrgy == "Barangay" && slctdIssue == "Type of Issue"){
		...
		bootbox.confirm({
      title: "BarangayAide:",
      message: "Proceeding does not mean that your complaint will automatically be posted. It will go through manual approving process by the barangay official. Complaint can be edited and deleted before approval process. Once approved you can not edit or delete. Note that faces of people can not be visible for data privacy consideration.",
      buttons: {
          cancel: {
      ...
      callback: function (result) {
        if(result == true){
          db.collection("barangay").where("name", "==", slctdBrgy)
                  ...
                  .then(function(querySnapshot) {
                      querySnapshot.forEach(function(doc) {
                          var brgy = doc.data().name;
                          var brgyID = doc.id;

                          db.collection("users").where("uid", "==", uidGet)
                          .get()
                          .then(function(querySnapshot) {
                              querySnapshot.forEach(function(doc) {
